...
with timestamp_prev <= timestamp. If there are multiple such values, it returns the value associated with the largest timestamp_prev.
...
public class TimeBasedKeyValueStore {
    ...
    static class Node implements Comparable<Node>
    {
        ...
        {
            return node.timestamp - this.timestamp;
        }
    ...
    Map<String, PriorityQueue<Node>> keyValueMap;
    ...
    public void set(String key, String value, int timestamp) {
        if(!keyValueMap.containsKey(key))
        {
            ...
            keyValueMap.put(key, pq);
        }
        else
        {
            PriorityQueue<Node> pq = keyValueMap.get(key);
            pq.add(new Node(value, timestamp));
            keyValueMap.put(key, pq);
        }
    }
