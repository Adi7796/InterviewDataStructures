...
import Design.LLD.AirlineBookingSystem.seat.SeatType;
...
public class AirlineManagementSystemDemo {
    ...
    public static void main(String[] args) {
        ...
        for (Flight flight : searchResults) {
            System.out.println("Flight: " + flight.getFlightNumber() + " - " + flight.getSourceCity() + " to " + flight.getDestinationCity());
        }

        Seat seat = new Seat("25A", SeatType.WINDOW);

        // Book a flight
        Booking booking = airlineManagementSystem.bookFlight(flight1, passenger1, seat, 100);
        if (booking != null) {
            System.out.println("Booking successful. Booking ID: " + booking.getBookingReference());
        } else {
            System.out.println("Booking failed.");
        }

        // Cancel a booking
        airlineManagementSystem.cancelBooking(booking.getBookingReference());
